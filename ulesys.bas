...
Dim a, i As Integer
a = 14
If Formchangeperson.cardhp(1).FontName <> "Bradley Gratis" Then
    For i = 1 To 2
        Formchangeperson.cardhp(i).FontSize = a
        Formchangeperson.cardatk(i).FontSize = a
        Formchangeperson.carddef(i).FontSize = a
    Next
End If
End Sub
...
If FormMainMode.PEStext1.FontName <> "Bradley Gratis" Then
    FormMainMode.PEStext1.FontSize = 16
    '===========PEAttackingForm
    For i = 1 To 3
...
'===========================
  ...
  Next
    ...
    Else
        FormMainMode.PEAttackingForm.Picture = LoadPicture(app_path & "gif\官方預設背景圖\" & Formsetting.對戰地圖選擇.ListIndex & ".jpg")
        FormMainMode.PEAttackingStartForm.Picture = LoadPicture(app_path & "gif\官方預設背景圖\" & Formsetting.對戰地圖選擇.ListIndex & ".jpg")
        FormMainMode.PEAttackingEndingForm.Picture = LoadPicture(app_path & "gif\官方預設背景圖\" & Formsetting.對戰地圖選擇.ListIndex & ".jpg")
    End If
    '=============================================
...
filenames = Dir(App.Path & "\character\", vbNormal)
Do While filenames <> ""
    人物系統類.卡片人物資訊讀入_初階段 App.Path & "\character\" & filenames
...
If Formsetting.checktest.Value = 0 Then
    ...
    FormMainMode.atkingnumtot.Visible = False
...
'================================
For i = 1 To 3
    FormMainMode.cardbackus(i).大人物圖片 = app_path & "gif\cardblack.png"
    FormMainMode.cardbackcom(i).大人物圖片 = app_path & "gif\cardblack.png"
Next
'---------以下是設定技能字體大小
If VBEPerson(1, 1, 2, 3, 5) = 0 Then
   FormMainMode.personatk(1).FontSize = VBEPerson(1, 1, 2, 3, 1)
   FormMainMode.personatk(2).FontSize = VBEPerson(1, 1, 2, 3, 2)
   FormMainMode.personatk(3).FontSize = VBEPerson(1, 1, 2, 3, 3)
   FormMainMode.personatk(4).FontSize = VBEPerson(1, 1, 2, 3, 4)
End If
If VBEPerson(2, 1, 2, 3, 5) = 0 Then
   FormMainMode.comaiatk(1).FontSize = VBEPerson(2, 1, 2, 3, 1)
   FormMainMode.comaiatk(2).FontSize = VBEPerson(2, 1, 2, 3, 2)
   FormMainMode.comaiatk(3).FontSize = VBEPerson(2, 1, 2, 3, 3)
   FormMainMode.comaiatk(4).FontSize = VBEPerson(2, 1, 2, 3, 4)
End If
'------------
...
FormMainMode.顯示列1.顯示列圖片 = app_path & "gif\DRAW.png"
'==================以下是音樂載入
FormMainMode.wmpse1.URL = app_path & "mp3\ulse06.mp3"
FormMainMode.wmpse1.Controls.stop
...
Next
戰鬥系統類.公用牌未使用檢查
FormMainMode.pageul = 公用牌各牌類型紀錄數(0, 2)
階段狀態數 = 0
...
For w = 1 To 角色人物對戰人數(1, 1)
    FormMainMode.cardus(w).Picture = LoadPicture(VBEPerson(1, w, 1, 5, 5))
...
For w = 1 To 角色人物對戰人數(2, 1)
    FormMainMode.cardcom(w).Picture = LoadPicture(VBEPerson(2, w, 1, 5, 5))
...
If Formsetting.chkpersonvsmode.Value = 1 Then
    For i = 2 To 3
        ...
        FormMainMode.cardcom(i).Picture = LoadPicture(app_path & "gif\personunknown.jpg")
    Next
End If
'--------------------------計算距離單位(HP血條)
...
turn = 1
FormMainMode.turni.Caption = turn
End Sub
Sub 自由戰鬥模式設定表單讀入程序()
Dim i, j As Integer
選單使用者事件 = True
...
FormMainMode.upjpg_2.大人物圖片 = App.Path & "\gif\startupjpg.png"
...
人物技能數字指示 = 0 '戰鬥系統表單-atkingnumtot.Caption的變數表示
...
Select Case uscom
    Case 1
        ...
        Formatkingus.Show 0, FormMainMode
    Case 2
        Formatkingcom.t1.Enabled = True
        Formatkingcom.Show 0, FormMainMode
End Select
End Sub
Sub 擲骰動畫表單顯示()
FormDice.FormActivate
FormDice.Show 0, FormMainMode
End Sub
