...
'         戰鬥系統類.座標計算_使用者手牌
        ...
        If Val(FormMainMode.pageul.Caption) < 事件卡記錄暫時數(1, 1) Then
           戰鬥系統類.執行動作_洗牌
        End If
    Case 3
         If 事件卡記錄暫時數(1, 2) > 事件卡記錄暫時數(1, 1) Or Val(FormMainMode.pageul.Caption) <= 0 Then
             turnpageonin = 1
...
Select Case 事件卡記錄暫時數(2, 3)
    ...
    Case 2
        ...
        FormMainMode.card(事件卡記錄暫時數(2, 4)).Picture = LoadPicture(app_path & "card\" & pagecardnum(事件卡記錄暫時數(2, 4), 8) & "-" & pageonin(事件卡記錄暫時數(2, 4)) & ".bmp")
        FormMainMode.wmpse7.Controls.play
        一般系統類.檢查音樂播放 7
        目前數(24) = 9
    ...
    Case 3
        ...
        '=====================
        事件卡記錄暫時數(2, 2) = 1
        If Val(FormMainMode.pageul.Caption) < 事件卡記錄暫時數(2, 1) Then
           戰鬥系統類.執行動作_洗牌
        End If
    Case 4
         If 事件卡記錄暫時數(2, 2) > 事件卡記錄暫時數(2, 1) Or Val(FormMainMode.pageul.Caption) <= 0 Then
             目前數(24) = 10
             FormMainMode.等待時間_2.Enabled = True
             事件卡記錄暫時數(2, 6) = 0
             Exit Sub
